...
function render_results_as_cards_grid($res, $req = []) {
  ...
  foreach ($res as $product) {
    if ( 
      /**
       * FILTER OUT drafts, inactives and sold outs products from results
       * except products withou invetory tracking 
       * TODO : Filter approppriate field
       */
      ($product->status == 'ACTIVE' && $product->totalInventory != '0') || 
      ($product->status == 'ACTIVE' && $product->type == 'Vino Cru') ||
      ($product->status == 'ACTIVE' && $product->type == 'Gift Cards') ||
      ($product->status == 'ACTIVE' && $product->type == 'Tastings') 
    ) { 
      /**
      ...
      foreach($product as $key => $value) {
        ...
        if ($key == "image") {
          ...
          $card_image = "<img src='$value' alt='product illustration'>";
        }
        if ($key == "handle") {
        ...
        if ($key == "type" && $value =="Vino Cru") {
          $card_action = "see";          
        }
      }
		/**
		 * Resolver card image link
		 */
		$card_image_link = "<a href='$card_href' title='See more info...'>$card_image</a>";
      
      /**
       * Resolve card action button
       */
      if ($card_action == "see") {
        $card_button = "<a href='$card_href' class='button default font-weight-bold text-transform guts__feed__card__button-see-product' style='width:100%; margin-top: .5em'>See product</a>";
      }
      else {
        $card_button = do_shortcode("[wps_products_buy_button hide_quantity='true' items_per_row='1' product_id='$card_id']");
      }
      /**
