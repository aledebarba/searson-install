...
class CPT_Model
{
    ...
    public function return_post_description($product) {
        return property_exists($product, 'body_html') ? $product->body_html : '';
    }

    public function return_post_date($product) {
        return property_exists($product, 'published_at') ? $product->published_at : '';
    }

    public function return_product_id($product) {

        if (property_exists($product, 'id')) {
            return Utils::convert_storefront_id_to_numeric($product->id);
        }
    ...
    public function set_product_model_defaults($product, $post_id)
    {
        ...
        $info = [
            ...
            // 'post_date' => $this->return_post_date($product),
        ...
        if (!$post_id) {
            $info['meta_input'] = [ 
                'product_id' => $this->return_product_id($product), 
    ...
    {
        $product_model = $this->set_product_model_defaults($product, $post_id);
