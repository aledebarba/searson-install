...
class Backend
{
    ...
    public function admin_scripts($hook)
    {
        ...
        if ($this->should_load_js() && !$this->is_wizard_page()) {
            ...
            if (\is_plugin_active('shopwp/shopwp.php')) {
                wp_die(
                    "Please deactivate the free version of ShopWP before activating ShopWP Pro. <br><br><a href=" .
                        admin_url('plugins.php') .
                        " class=\"button button-primary\">Back to plugins</a>."
                );
            }
            ...
            $vendors_admin_path = 'dist/vendors-admin.d307a507.min.js';
            ...
            $main_path = 'dist/admin.d307a507.min.js';
            $main_filetime = filemtime(SHOPWP_PLUGIN_DIR_PATH . $main_path) . time();

            $main_url = SHOPWP_PLUGIN_URL . $main_path;

            wp_enqueue_script('shopwp-runtime', $runtime_url, [], $runtime_filetime);
            wp_enqueue_script('shopwp-vendors-admin', $vendors_admin_url, [], $vendors_admin_filetime);
   ...
   */
    public function add_dashboard_menus()
    {
        ...
        if (apply_filters('shopwp_show_dashboard', current_user_can('edit_pages'), $user)) {
            ...
            $plugin_name = SHOPWP_PLUGIN_NAME_FULL_PRO;
    ...
    public function shopwp_blocks_assets()
    {
        if (is_admin()) {
            ...
            $runtime_path = 'dist/runtime.d307a507.min.js';
            $runtime_filetime = filemtime(SHOPWP_PLUGIN_DIR_PATH . $runtime_path);
            $runtime_url = SHOPWP_PLUGIN_URL . 'dist/runtime.d307a507.min.js';

            $vendors_admin_path = 'dist/vendors-admin.d307a507.min.js';
            $vendors_admin_filetime = filemtime(SHOPWP_PLUGIN_DIR_PATH . $vendors_admin_path);
            $vendors_admin_url = SHOPWP_PLUGIN_URL . $vendors_admin_path;
            
            $main_path = 'dist/blocks.d307a507.min.js';
    ...
    public function shopwp_wizard_assets()
    {
        if ($this->is_wizard_page()) {
            ...
            $runtime_url = SHOPWP_PLUGIN_URL . 'dist/runtime.d307a507.min.js';
            $vendors_admin_url =
                SHOPWP_PLUGIN_URL . 'dist/vendors-admin.d307a507.min.js';
            $main_url = SHOPWP_PLUGIN_URL . 'dist/wizard.d307a507.min.js';
   ...
   public function add_edit_in_shopify_button() {
      ...
      <div class="wrap">
         ...
         <a href="<?= esc_url_raw($link_href); ?>" target="_blank" class="page-title-action show">Edit in Shopify</a>
      </div>
    ...
    public function init()
    {
        ...
        if (is_admin()) {
            ...
            add_action('enqueue_block_assets', [$this, 'shopwp_blocks_assets']);

            add_filter(
                'block_categories_all',
                [$this, 'shopwp_block_categories'],
                10,
                2
            );
